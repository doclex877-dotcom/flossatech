...
export default function Home() {
  ...
  return (
    <div>
      <section className="border-b-2 border-[var(--color-ink)] bg-[var(--color-bench)] text-[var(--color-paper)]">
        <div className="max-w-6xl mx-auto px-5 py-16 md:py-24 grid md:grid-cols-[1.3fr_0.7fr] gap-10 items-end">
          <div>
      ...
      <section className="max-w-6xl mx-auto px-5 py-14">
        ...
        {lead && (
          <Link href={`/blog/${lead.slug}`} className="group grid md:grid-cols-[1fr_auto] gap-6 items-center border-2 border-[var(--color-ink)] p-6 md:p-8 mb-10 bg-white hover:bg-[var(--color-paper-2)] transition-colors">
            <div>
              ...
              <p className="text-[var(--color-muted)] max-w-xl">{lead.dek}</p>
            </div>
            {typeof lead.verdictScore === "number" && (
              <VerdictDial score={lead.verdictScore} label="Value Score" size={110} />
            )}
          </Link>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {rest.map((a) => (
            <Link
              key={a.slug}
              href={`/blog/${a.slug}`}
              className="group border-2 border-[var(--color-ink)] p-6 bg-white hover:bg-[var(--color-paper-2)] transition-colors flex flex-col"
            >
              <div className="font-[var(--font-mono)] text-xs uppercase tracking-widest text-[var(--color-signal)] mb-2">
                {a.category}
              </div>
